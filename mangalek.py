...
MAX_PAGES_SAFETY = 3     # عدد صفحات الفهرس لتغطية مكتبة مانجا ليك
...
def extract_chapters_mangalik(session, manga_url: str) -> dict:
    """استخراج خريطة الفصول عبر AJAX الخاص بقالب Madara أو عبر الصفحة المباشرة"""
    clean_url = normalize_url(manga_url)
    ajax_url = f"{clean_url}/ajax/chapters/"
    elements = []

    # 1. طلب AJAX السريع لمانجا ليك
    try:
...
def sync_mangalik_fast():
    ...
    for index, item in enumerate(top_targets, 1):
        ...
        try:
            ...
            print(f"✓ [{index}/{len(top_targets)}] تم تجهيز مانجا ليك: {slug} ({len(details['chapters'])} فصل)")
            time.sleep(0.3)
        except Exception as e:
    ...
    print(f"\n⚡ اكتملت مزامنة مانجا ليك الخاطفة! تم الحفظ في {CATALOG_FILE}")
